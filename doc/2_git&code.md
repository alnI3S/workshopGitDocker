# Démarrer avec Git et VSCode

## 1️⃣ Identification et authentificaton

- **Configurer Git** (`git config`) : Définir votre identité (nom, email) pour que vos contributions soient reconnues.
- **Tokens d’authentification** : Sécuriser vos interactions avec les dépôts distants (GitHub, GitLab) pour pousser (`push`) ou récupérer (`pull`) du code.

## 2️⃣ Gérer le travail efficacement

- **basique**: commencer avec  un projet local.
- **Outils de diff et merge**: Visualiser et résoudre les conflits directement dans VSCode.
...
## 3️⃣ Partager le code sur GitHub
